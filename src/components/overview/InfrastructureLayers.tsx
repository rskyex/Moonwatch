  ...
  return (
    <div className="space-y-3">
      {layers.map(({ category, label, items }) => (
        <div key={category} className="flex items-baseline gap-6 py-2.5 border-b border-border/40">
          <span className="text-[12px] text-cold/60 tracking-wide w-40 shrink-0 font-medium">
            {label}
          </span>
          <div className="flex-1 flex flex-wrap items-baseline gap-x-4 gap-y-1.5">
            {items.slice(0, 4).map(i => (
              <Link
                ...
                className="text-[13px] text-foreground hover:text-cold transition-colors"
              >
                {i.name}
              </Link>
            ))}
            {items.length > 4 && (
              <span className="text-[11px] text-muted">+{items.length - 4}</span>
            )}
          </div>
        </div>
      ))}
